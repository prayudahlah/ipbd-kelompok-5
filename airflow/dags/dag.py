...
def scrape_harga_minyak(**context):
    ...
    print(
        f"[TASK 2] Total keseluruhan: {len(df)} baris "
    ...
    print(
        f"[TASK 2] Rentang: {df_20y['date'].min().date()} s/d {df_20y['date'].max().date()}"
    )
...
def scrape_berita_ev(**context):
    ...
    formatted_start = datetime.strptime(START_DATE_DATA, "%Y-%m-%d").strftime(
        "%m/%d/%Y"
    )
    ...
    print(
        f"[TASK 3] Mencari: '{keyword}' dari {formatted_start} hingga {formatted_end}..."
    )
